...
LIBHIO_CLIENT5(hio_setsockopt, __NR_setsockopt, int, int, int, int, void *, socklen_t);
...
*/
 ...
 if(hio_setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one)) < 0)
 {
 ...
 {
     if(hio_setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, &(p->prot.sndbufsz), 
 ...
 if( p->tr ) {                             /* Primary transmitter */
   ...
   if (hio_setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(int))) {
     printf("NetPIPE: server: unable to setsockopt -- errno %d\n", errno);
     exit(557);
   }
   printf("setsockopt %d SO_REUSEADDR\n", sockfd);
   ...
   if (hio_bind(sockfd, (struct sockaddr *) lsin1, sizeof(*lsin1)) < 0){
...
readFully(int fd, void *obuf, int len)
{
  ...
  while (bytesLeft > 0 &&
         (bytesRead = hio_read(fd, (void *) buf, bytesLeft)) > 0)
    {
      bytesLeft -= bytesRead;
      buf += bytesRead;
    }
...
void Sync(ArgStruct *p)
{
    ...
    if (hio_write(p->commfd, s, strlen(s)) < 0 ||           /* Write to nbor */
...
void SendData(ArgStruct *p)
{
    ...
    q = p->s_ptr;
    while (bytesLeft > 0 &&
           (bytesWritten = hio_write(p->commfd, q, bytesLeft)) > 0)
      {
...
void RecvData(ArgStruct *p)
{
    ...
    q = p->r_ptr;
    while (bytesLeft > 0 &&
           (bytesRead = hio_read(p->commfd, q, bytesLeft)) > 0)
      {
        bytesLeft -= bytesRead;
        q += bytesRead;
      }
...
void SendTime(ArgStruct *p, double *t)
{
    ...
    ntime = htonl(ltime);
    if (hio_write(p->commfd, (char *)&ntime, sizeof(uint32_t)) < 0)
      {
...
void SendRepeat(ArgStruct *p, int rpt)
{
  ...
  nrpt = htonl(lrpt);
  if (hio_write(p->commfd, (void *) &nrpt, sizeof(uint32_t)) < 0)
    {
...
void establish(ArgStruct *p)
{
  ...
  if( p->tr ){
    ...
    while( hio_connect(p->commfd, (struct sockaddr *) &(p->prot.sin1),
    ...
    /* SERVER */
    hio_listen(p->servicefd, 5);
    p->commfd = hio_accept(p->servicefd, (struct sockaddr *) &(p->prot.sin2), &clen);
...
*/
    ...
    if(hio_setsockopt(p->commfd, IPPROTO_TCP, TCP_NODELAY,
                  &one, sizeof(one)) < 0)
    {
...
/*           p->prot.sndbufsz);*/
      if(hio_setsockopt(p->commfd, SOL_SOCKET, SO_SNDBUF, &(p->prot.sndbufsz), 
                                       sizeof(p->prot.sndbufsz)) < 0)
      {
        printf("setsockopt: SO_SNDBUF failed! errno=%d\n", errno);
        exit(556);
      }
      if(hio_setsockopt(p->commfd, SOL_SOCKET, SO_RCVBUF, &(p->prot.rcvbufsz), 
                                       sizeof(p->prot.rcvbufsz)) < 0)
      {
...
void CleanUp(ArgStruct *p)
{
   ...
   if (p->tr) {
      ...
      hio_write(p->commfd,quit, 5);
      hio_read(p->commfd, quit, 5);
      close(p->commfd);
