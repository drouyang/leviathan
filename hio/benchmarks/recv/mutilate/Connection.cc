  ...
  hostname(_hostname), port(_port), base(_base), evdns(_evdns)
{
  ...
  */
  prot = new ProtocolEcho(options, this, bev);
...
void Connection::issue_something(double now) {
  ...
  */
  uint64_t buffersize = atoi(options.valuesize);
  char *buf = (char *)malloc(buffersize);
  issue_msg(buf, buffersize, now);
  free(buf);
}
 ...
 */
void Connection::issue_msg(const char* buf, int length, double now) {
  Operation op;
  int l;
